...
function createRoot(fn, detachedOwner) {
  const listener2 = Listener, owner = Owner, unowned = fn.length === 0, current = detachedOwner === void 0 ? owner : detachedOwner, root = unowned ? {
    owned: null,
    cleanups: null,
    context: null,
    owner: null
...
function createResource(pSource, pFetcher, pOptions) {
  ...
  function load(refetching = true) {
    ...
    return p2.then(
      ...
      (e) => loadEnd(p2, void 0, castError(e), lookup)
    );
  }
...
function runComputation(node, value, time) {
  let nextValue;
  const owner = Owner, listener2 = Listener;
  Listener = Owner = node;
  try {
...
function mapArray(list, mapFn, options = {}) {
  ...
  return () => {
    ...
    newItems[$TRACK];
    return untrack(() => {
      let newIndices, newIndicesNext, temp, tempdisposers, tempIndexes, start, end2, newEnd, item;
      if (newLen === 0) {
        if (len !== 0) {
          dispose(disposers);
          disposers = [];
        ...
        for (start = 0, end2 = Math.min(len, newLen); start < end2 && items[start] === newItems[start]; start++) ;
        for (end2 = len - 1, newEnd = newLen - 1; end2 >= start && newEnd >= start && items[end2] === newItems[newEnd]; end2--, newEnd--) {
...
function mergeProps(...sources) {
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    sources[i] = typeof s2 === "function" ? (proxy = true, createMemo(s2)) : s2;
  }
...
function insert(parent, accessor, marker, initial2) {
  if (marker !== void 0 && !initial2) initial2 = [];
  if (typeof accessor !== "function") return insertExpression(parent, accessor, initial2, marker);
  createRenderEffect((current) => insertExpression(parent, accessor(), current, marker), initial2);
}
...
function Button(props) {
  ...
  return (() => {
    ...
    spread(_el$, mergeProps(props, {
      get ["class"]() {
        return `${variantClasses[props.variant || "primary"]} ${props.class || ""}`;
      }
    }), false);
    return _el$;
  })();
}
var _tmpl$ = /* @__PURE__ */ template(`<button class=theme-switch aria-label="Toggle theme"><span class=theme-switch__icon>`);
function ThemeSwitch() {
  ...
  return (() => {
    ...
    addEventListener(_el$, "click", toggleTheme, true);
    insert(_el$2, () => isDarkMode() ? "🌙" : "☀️");
    createRenderEffect(() => setAttribute(_el$, "aria-pressed", isDarkMode()));
    return _el$;
  })();
}
...
library$1.add(faArrowRight, faArrowLeft, faChartPie, faCog, faCheckCircle, faFacebook, faYoutube, faInstagram, faSquareXTwitter, faTiktok, faSnapchat, faGlobe, faPlus, faEdit, faTrash);
dom$1.watch();
export {
  Button as B,
  For as F,
...
//# sourceMappingURL=IconSetup-7JOCPL7z.js.map
